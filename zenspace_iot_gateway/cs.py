...
class CSClient(object):
    ...
    def delete(self, base, query='', tree=0):
        ...
        if sys.platform == 'linux2':
            cmd = "delete\n{}\n{}\n{}\n".format(base, query, tree)
            log.debug("delete command is --{}".format(cmd))
            return self._dispatch(cmd)
        else:
            # Running in a computer so use http to send the put to the device.
            import requests
    ...
    def post(self, base, value='', query='', tree=0):
        """
        Constructs and sends a put request to update or add specified data to the device router tree.
    ...
    def iot_ip():
        ...
        settings_file = os.path.join(os.path.dirname(os.getcwd()), '/resources/ip.ini')
        config = configparser.ConfigParser()
        config.read(settings_file)

        #key in sdk_settings.ini
        # sdk_key = 'sdk'
        iot_ip_key = 'iot_gateway_ip'
        # if sdk_key in config:
        if iot_ip_key in config:
                 iot_ip = config[iot_ip_key]
                 # CSClient.log('mqtt','iot ip is - {}'.format(iot_ip))
                 print('mqtt','iot ip is - {}'.format(iot_ip))
        else:
                 # CSClient.log('mqtt','ERROR 1: The {} key does not exist in {}'.format(iot_ip_key, settings_file))
                 print('mqtt','ERROR 1: The {} key does not exist in {}'.format(iot_ip_key, settings_file))
    ...
    def _dispatch(self, cmd):
        ...
        try:
            ...
            log.debug("err message is --{}".format(errmsg))
